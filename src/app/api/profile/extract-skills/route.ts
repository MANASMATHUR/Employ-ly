...
export async function POST(request: NextRequest) {
    try {
        ...
        if (sanitizedText.trim().length < 10) {
            throw new ValidationError('Please provide at least 10 characters of text to analyze');
        }

        const result = await extractSkills(sanitizedText);

        logger.info('Skills extracted', { userId: auth.userId, skillCount: result.skills.length });
